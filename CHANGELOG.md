...
- File activity reports render in two tabs: Summary (attribution, databases, changed files) and Individual Files — every watched file with all of its stats in a sortable, searchable table with a JSON download of the current view, built on the same table machinery as the HTTP request tables. Paths display relative to the Signal K data directory with a copy-full-path button, and a default-on "Hide readonly files" filter keeps the writers in focus
