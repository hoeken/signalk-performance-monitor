...
## Feature 6: Load testing

A load generator on its own page, so the metrics and profiles above can be read against a known, repeatable load instead of whatever the boat happens to be doing. The first (and so far only) tester publishes deltas; the test-type select is the seam further testers plug into.

**Delta publishing** (`webapp/src/loadTest.ts`, `DeltaLoadTest`) runs **entirely in the browser**: it opens a websocket to `/signalk/v1/stream?subscribe=none` on the serving origin and publishes `{ context: "vessels.self", updates: [{ values: [...] }] }` messages of random numbers. That is deliberate — the deltas travel the same inbound path a real NMEA or plugin producer's would (websocket parse → `handleMessage` → full-model update → subscription fan-out), so a profile captured during a run measures the server's delta pipeline rather than a plugin calling itself. It also needs no backend code: cookies authenticate the socket the same way they authenticate the plugin's own routes, and the server's `processUpdates` assigns the `ws` source.

A **delta** here is one path/value update — the unit of work the server does — and a websocket **message** is the envelope carrying one or more of them. The run is configured by the delta rate, and the batch size divides it into the message rate (`messageRateOf`), so batching trades messages for envelope overhead while holding the load constant — the comparison the knob exists for.

Parameters: **test duration** (default 30 s), **number of paths** (500), **delta rate** in Hz (500), and **deltas per message** (1), plus a **profile during test** choice of none / CPU / memory. Every field is clamped to a positive integer, and deltas per message can never exceed the path count (a message must not list the same path twice). Paths are `testing.0` … `testing.<n-1>`, cycled round-robin so each gets an equal share; the ids are the index rather than a fresh random number per run because the server keeps every path it has ever seen in its full model — stable ids mean reruns reuse the same paths instead of growing the model forever.

Rate is held by a **catch-up accumulator**, not one timer per message: browsers clamp `setInterval` to ~4 ms and throttle background tabs hard, so a 20 ms tick sends however many messages the wall clock says are due. The accumulator counts in deltas (the configured unit) and sends whole messages only — a partial message's worth stays as debt for the next tick rather than being rounded up, which is what keeps a fractional message rate (500 deltas/s at 3 per message is 166.67 msg/s) exact over the run. Two safety valves keep the tester honest rather than bursty: a per-tick cap of four ticks' worth of messages, and a stop-sending threshold at 1 MB of `ws.bufferedAmount`. Both write the shortfall off to a visible **Skipped** counter (never carried as debt, which would grow unboundedly) — so a browser that can't keep up, or a server that's fallen behind, is reported instead of hidden. Live stats report **both units**: deltas successfully sent (with the message count that carried them), achieved deltas/s and messages/s, bytes sent, and deltas skipped. Stats are pushed to the UI every 250 ms regardless of send rate, so a 5000 Hz run doesn't re-render per message.

When a profile is selected, the capture is started from the engine's `beforeRun` hook — after the socket opens, before the first delta — so the capture window covers the load and not the connect handshake, and a rejected start (409 capture busy, duration over the configured cap) aborts the run with the server's own message. A run is bound to the page: leaving it closes the socket rather than leaving a hidden load on the server.
...
- **Pages** — a nav bar (`NavBar`) switches between **Monitor** (`MonitorPage`: everything below) and **Load Testing** (`LoadTestPage`, Feature 6). Routing is by hash (`webapp/src/route.ts`, `#/monitor` / `#/load-test`) so pages are linkable and survive a reload without a router: the webapp is mounted at an arbitrary path under the server, which rules out history routing unless the server learns to serve `index.html` for unknown sub-paths. The shell (`App`) owns the metrics poll (feeding the header's "updated" stamp and the Monitor tiles) and the auth-error banner; page state belongs to the pages. The load-test page deliberately shows no metric tiles — it is the instrument, not the readout.
...
- **Component tests** (`webapp/src/*.test.tsx`) — React Testing Library (jsdom) for the metrics tiles, app flow, page navigation, and report table, rendered from fixture API responses.
- **Load-test engine tests** (`webapp/src/loadTest.test.ts`) — the delta sender driven by fake timers against a stand-in socket: achieved rate, batching (100 deltas/s at 5 per message is 20 messages/s) and the partial-message debt that keeps a fractional message rate exact, message shape and round-robin path coverage, byte/delta accounting, self-stop at the duration, `beforeRun` ordering and failure, backpressure skipping, the throttled-tab burst cap (the wall clock jumped forward with no tick firing), and early stop.
